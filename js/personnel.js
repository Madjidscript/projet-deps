$(document).ready(function () {
        ...
        if (file) {
            ...
            reader.onload = function (e) {
                ...
                rows.forEach((row, index) => {
                    ...
                    if (columns.length === 11) { // Vérifie que les colonnes sont valides
                        table.row.add([
    ...
    $('#downloadBtn').on('click', function () {
        let csv = 'N°,MATRICULE, Nom et Prénoms, Sexe (1), Date de naissance, Pays (2), Diplôme, Grade, Statut (3), Date 1ère prise de service, Fonction';
        table.rows().data().each(function (row) {
            csv += row.slice(0, 11).join(',') + '\n'; // Exclure les boutons d'action
        });
